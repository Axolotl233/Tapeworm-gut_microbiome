plot_ancom_diff_res <- function(x,y){
  #x = ancombc2_res_tab
  tmp_data <- x
  tmp_data$taxon_class <- factor(tmp_data$taxon_class,levels = rev(sort(unique(tmp_data$taxon_class))))
  p1 <- ggplot()+
    geom_hline(aes(yintercept = -log10(0.05)) ,linetype = "dashed",color = "grey40")+
    geom_point(data = tmp_data,aes(x=lfc,
                                   y=log_pvalue,
                                   size=log_count,
                                   color=class,
    ))+
    ...
    geom_text_repel(data = tmp_data %>% arrange(pvalue) %>% filter(pvalue < 0.05 & prevalence_rate > 0.2) %>%
                      filter(taxon_class != "Unclassified")
                    ,aes(x=lfc,y=log_pvalue,label = taxon_id))
  #tmp_out_1 <- gsub("csv$","pdf",x)
  tmp_out_1 <- y
  ggsave(tmp_out_1,p1,width = 8,height = 6)
}
...
load("workfile/z.phyloseq.ancombc.Rdata")
dir.create("output/different_analysis",showWarnings = F)
out_dir <- "output/different_analysis"


phyloseqin_count_used <- subset_samples(phyloseqin_count_ancombc,groups %in% group[c(1,2)])
tmp_meta <- metadatadf %>%
...
ancombc2_out = ancombc2(data =  phyloseqin_count_used,  p_adj_method = "fdr",
                        ...
                        prv_cut = 0, lib_cut = 1000, struc_zero = F, neg_lb = F,
                        iter_control = list(tol = 1e-5, max_iter = 20,verbose=T),
                        em_control = list(tol = 1e-5, max_iter = 100), 
                        alpha = 0.05,global = F,verbose = T)
...
colnames(ancombc2_res_tab) <-  c("taxon_id","lfc","se","w","pvalue","qvalue","diff","pass")
...
plot_ancom_diff_res("output/2.differece_ancombc2.20_hc_vs_infect.csv")
...
View(ancombc2_res_tab %>% filter (Phylum == "Bacteroidetes") %>% group_by(Genus,class) %>% summarise(n()))
ancombc2_res_tab %>% group_by(class,Phylum) %>% summarise(n())
#plot_ancom_diff_res(ancombc2_res_tab,"output/CE.differece_ancombc2.40.pdf")


### ancombc2 ancombc 2.20 long
...
ancombc2_out = ancombc2(data =  phyloseqin_count_used,  p_adj_method = "fdr",
                        ...
                        tax_level = "Species",
                        rand_formula = "(1|pair)",
                        prv_cut = 0.1, lib_cut = 0, struc_zero = T, neg_lb = F,
...
write_csv(ancombc2_res_tab_long,"output/2.differece_ancombc2.20_long.csv")



phyloseqin_count_used <- subset_samples(phyloseqin_count,enterotype_class %in% c("health_ET1","infected_ET1"))
tmp_meta <- metadatadf %>% filter(sample %in% phyloseqin_count_used@sam_data$sample)
ancombc2_out = ancombc2(data =  phyloseqin_count_used,  p_adj_method = "fdr",
                        fix_formula = "enterotype_class+age+sex+bmi",group = "enterotype_class", tax_level = "Species",
                        prv_cut = 0.1, lib_cut = 100, struc_zero = T, neg_lb = F,
                        iter_control = list(tol = 1e-5, max_iter = 20,verbose=T),
                        em_control = list(tol = 1e-5, max_iter = 100), 
                        alpha = 0.05,global = F,verbose = T,pairwise = T)
...
ancombc2_str_zero <- ancombc2_out$zero_ind
ancombc2_res_tab <- phase_ancom_diff_res(ancombc2_res_tab,tmp_otu,tmp_meta,
                                         "health_ET1","infected_ET1",group_col = 17)
ancombc2_res_tab <- phase_ancom_add_tax(tab = ancombc2_res_tab,tax = tmp_tax,target_class = c("Phylum","Genus"))
ancombc2_res_tab_ET1 <- ancombc2_res_tab
write_csv(ancombc2_res_tab_ET1,"output/2.differece_ancombc2.20_ET1.csv")



phyloseqin_count_used <- subset_samples(phyloseqin_count,enterotype_class %in% c("health_ET2","infected_ET2"))
tmp_meta <- metadatadf %>% filter(sample %in% phyloseqin_count_used@sam_data$sample)
ancombc2_out = ancombc2(data =  phyloseqin_count_used,  p_adj_method = "fdr",
                        fix_formula = "enterotype_class+age+sex+bmi",group = "enterotype_class", tax_level = "Species",
                        prv_cut = 0.1, lib_cut = 100, struc_zero = T, neg_lb = F,
...
ancombc2_str_zero <- ancombc2_out$zero_ind
ancombc2_res_tab <- phase_ancom_diff_res(ancombc2_res_tab,tmp_otu,tmp_meta,
                                         "health_ET2","infected_ET2",group_col = 17)
ancombc2_res_tab <- phase_ancom_add_tax(tab = ancombc2_res_tab,tax = tmp_tax,target_class = c("Phylum","Genus"))
ancombc2_res_tab_ET2 <- ancombc2_res_tab
write_csv(ancombc2_res_tab_ET2,"output/2.differece_ancombc2.20_ET2.csv")
...
save(ancombc2_res_tab_ET1,ancombc2_res_tab_ET2,
     ...
     file = "workfile/z.diff_res.2.20.Rdata")
